...
/**
 * Methods for the Anderson-Darling distribution of the *Anderson-Darling* test
 * statistic {@code A_n^2} given an ordered sample of {@code n} independent
 * uniforms {@code U_i} over {@code (0,1)}.
 * <p>
...
public final class AndersonDarling {
     ...
     * bar(F)<sub>n</sub>(<em>x</em>) with parameter {@code n}, which is
     * {@code P[A_n^2 >= x]}.
     *
     * @param n
     ...
     * @return {@code P[A_n^2 >= x]}
     * @throws IllegalArgumentException
     *             if {@code n} is not strictly positive
     */
    public static double barF(int n, double x) {
        ...
        double q;
        if (x > 5.0) {
            // Grace-Wood approximation in upper tail
            double nd = n;
        ...
        double res = (F2AD[i - 2] - 2.0 * F2AD[i - 1] + F2AD[i]) * q * (q + 1.0) / 2.0 + (F2AD[i] - F2AD[i - 1]) * q
                + F2AD[i];
    ...
    /**
     * Computes the {@code Anderson-Darling} distribution function
     * {@code F_n(x)} at {@code x} for sample size {@code n}.
     * <p>
     * For
     * {@code 0.2 < x < 5}, the asymptotic distribution was first computed by
     * numerical integration; then a linear correction {@code O(1/n)} obtained
     * by simulation was added. For {@code x > 5}, the Grace-Wood empirical
     * approximation is used. For
     * {@code x < 0.2}, the Marsaglias' approximation is used.
     *
     * For {@code n > 6}, the method gives at least 3 decimal digits of
     * precision except for small {@code x}; for {@code n <= 6}, it gives at
     * least 2 decimal digits of precision except for small {@code x}. For
     * {@code n = 1}, the exact formula is used.
     *
     * @param n
     *            the sample size, {@code 1} or greater
     * @param x
    ...
    private static double cdf_N_1(double x) {
    ...
    private static double barF_N_1(double x) {
    ...
    private static double lower_Marsaglia(int n, double x) {
        // the lower tail of Marsaglia's algorithm for n = infinity
        double p0 = (2.00012 + (0.247105 - (0.0649821 - (0.0347962 - (0.011672 - 0.00168691 * x) * x) * x) * x) * x);
        p0 *= Math.exp(-1.2337141 / x) / Math.sqrt(x);
        return p0 >= 0.0 ? p0 : 0.0;
    }

    // x infinity for Anderson-Darling
    private static final double AD_BIG = 1000.0;

    // Tables for the approximation of the Anderson-Darling distribution
    private static final double[] F2AD = new double[103];
    private static final double[] CoAD = new double[103];
