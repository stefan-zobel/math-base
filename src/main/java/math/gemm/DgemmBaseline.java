...
public final class DgemmBaseline {
    ...
    private static void aTimesBRange(int m, int jFrom, int jTo, int k, double alpha, double[] a, int _a_offset, int lda, double[] b, int _b_offset, int ldb, double beta, double[] c, int _c_offset, int ldc) {
        ...
        for (int j = jFrom; j < jTo; j += NC) {
            ...
            for (int p = 0; p < k; p += KC) {
                ...
                double currentBeta = (p == 0) ? beta : 1.0;
    ...
    // Inner block kernel
    private static void kernel_aTimesB(int m, int n, int k, double alpha, double[] a, int _a_offset, int lda, double[] b, int _b_offset, int ldb, double beta, double[] c, int _c_offset, int ldc) {
        // Form C := alpha*A*B + beta*C
        ...
        for (int o = n; o > 0; o--) {
            ...
            if (alpha == 1.0) {
                for (; p >= 4; p -= 4) {
                    ...
                    double tmp0 = b[(w - 1) + bColOffset];
                    double tmp1 = b[w + bColOffset];
                    double tmp2 = b[(w + 1) + bColOffset];
                    double tmp3 = b[(w + 2) + bColOffset];
                    int i = 0;
                ...
                for (; p >= 4; p -= 4) {
                    ...
                    int aOffset2 = (w + 1) * lda + _a_offset;
                    int aOffset3 = (w + 2) * lda + _a_offset;
                    double tmp0 = alpha * b[(w - 1) + bColOffset];
                    double tmp1 = alpha * b[w + bColOffset];
                    double tmp2 = alpha * b[(w + 1) + bColOffset];
                    double tmp3 = alpha * b[(w + 2) + bColOffset];
                    int i = 0;
                    for (; i < mLimit; i += UNROLL) {
                        c[i      + cOffset] += tmp0 * a[i      + aOffset0] + tmp1 * a[i      + aOffset1] + tmp2 * a[i      + aOffset2] + tmp3 * a[i      + aOffset3];
                        c[i +  1 + cOffset] += tmp0 * a[i +  1 + aOffset0] + tmp1 * a[i +  1 + aOffset1] + tmp2 * a[i +  1 + aOffset2] + tmp3 * a[i +  1 + aOffset3];
                ...
                for (; p > 0; p--) {
                    int aOffset = (w - 1) * lda + _a_offset;
                    double tmp = alpha * b[(w - 1) + bColOffset];
                    int i = 0;
                    for (; i < mLimit; i += UNROLL) {
                        c[i      + cOffset] += tmp * a[i      + aOffset];
                        c[i +  1 + cOffset] += tmp * a[i +  1 + aOffset];
    ...
    private static void aTransTimesBRange(int m, int jFrom, int jTo, int k, double alpha, double[] a, int _a_offset, int lda, double[] b, int _b_offset, int ldb, double beta, double[] c, int _c_offset, int ldc) {
        ...
        for (int u = jFrom + 1; u <= jTo; u++) {
            ...
            for (; w <= m - 4; w += 4) {
                ...
                double s0a = 0.0, s0b = 0.0;
                double s1a = 0.0, s1b = 0.0;
                double s2a = 0.0, s2b = 0.0;
                double s3a = 0.0, s3b = 0.0;
                int i = 0;
                for (; i < kLimit; i += UNROLL) {
                    double b0  = b[i      + bOffset];
                    double b1  = b[i +  1 + bOffset];
                    double b2  = b[i +  2 + bOffset];
                    double b3  = b[i +  3 + bOffset];
                    double b4  = b[i +  4 + bOffset];
                    double b5  = b[i +  5 + bOffset];
                    double b6  = b[i +  6 + bOffset];
                    double b7  = b[i +  7 + bOffset];
    ...
    // Inner block kernel
    private static void kernel_aTimesBTrans(int m, int n, int k, double alpha, double[] a, int _a_offset, int lda, double[] b, int _b_offset, int ldb, double beta, double[] c, int _c_offset, int ldc) {
        final int mLimit = m - UNROLL + 1;
        int u = 1;
        for (int o = n; o > 0; o--) {
            int cOffset = (u - 1) * ldc + _c_offset;
            ...
            if (alpha == 1.0) {
                for (; p >= 4; p -= 4) {
                    ...
                    int aOffset2 = (w + 1) * lda + _a_offset;
                    int aOffset3 = (w + 2) * lda + _a_offset;
                    double tmp0 = b[(u - 1) + (w - 1) * ldb + _b_offset];
                    double tmp1 = b[(u - 1) + w * ldb + _b_offset];
                    double tmp2 = b[(u - 1) + (w + 1) * ldb + _b_offset];
                    double tmp3 = b[(u - 1) + (w + 2) * ldb + _b_offset];
                    int i = 0;
                    for (; i < mLimit; i += UNROLL) {
                        c[i      + cOffset] += tmp0 * a[i      + aOffset0] + tmp1 * a[i      + aOffset1] + tmp2 * a[i      + aOffset2] + tmp3 * a[i      + aOffset3];
                        c[i +  1 + cOffset] += tmp0 * a[i +  1 + aOffset0] + tmp1 * a[i +  1 + aOffset1] + tmp2 * a[i +  1 + aOffset2] + tmp3 * a[i +  1 + aOffset3];
                ...
                for (; p > 0; p--) {
                    int aOffset = (w - 1) * lda + _a_offset;
                    double tmp = b[(u - 1) + (w - 1) * ldb + _b_offset];
                    int i = 0;
                    for (; i < mLimit; i += UNROLL) {
                        c[i      + cOffset] += tmp * a[i      + aOffset];
                        c[i +  1 + cOffset] += tmp * a[i +  1 + aOffset];
                ...
                for (; p >= 4; p -= 4) {
                    ...
                    int aOffset2 = (w + 1) * lda + _a_offset;
                    int aOffset3 = (w + 2) * lda + _a_offset;
                    double tmp0 = alpha * b[(u - 1) + (w - 1) * ldb + _b_offset];
                    double tmp1 = alpha * b[(u - 1) + w * ldb + _b_offset];
                    double tmp2 = alpha * b[(u - 1) + (w + 1) * ldb + _b_offset];
                    double tmp3 = alpha * b[(u - 1) + (w + 2) * ldb + _b_offset];
                    int i = 0;
                    for (; i < mLimit; i += UNROLL) {
                        c[i      + cOffset] += tmp0 * a[i      + aOffset0] + tmp1 * a[i      + aOffset1] + tmp2 * a[i      + aOffset2] + tmp3 * a[i      + aOffset3];
                        c[i +  1 + cOffset] += tmp0 * a[i +  1 + aOffset0] + tmp1 * a[i +  1 + aOffset1] + tmp2 * a[i +  1 + aOffset2] + tmp3 * a[i +  1 + aOffset3];
    ...
    private static void aTransTimesBTransRange(int m, int jFrom, int jTo, int k, double alpha, double[] a, int _a_offset, int lda, double[] b, int _b_offset, int ldb, double beta, double[] c, int _c_offset, int ldc) {
        ...
        for (int u = jFrom + 1; u <= jTo; u++) {
            ...
            for (; w <= m - 4; w += 4) {
                ...
                int i = 0;
                for (; i < kLimit; i += UNROLL) {
                    double b0  = b[bOffset + (i     ) * ldb];
                    double b1  = b[bOffset + (i +  1) * ldb];
                    double b2  = b[bOffset + (i +  2) * ldb];
                    double b3  = b[bOffset + (i +  3) * ldb];
                    double b4  = b[bOffset + (i +  4) * ldb];
                    double b5  = b[bOffset + (i +  5) * ldb];
                    double b6  = b[bOffset + (i +  6) * ldb];
                    double b7  = b[bOffset + (i +  7) * ldb];
                    s0a += a[i      + aOffset0] * b0  + a[i +  1 + aOffset0] * b1  + a[i +  2 + aOffset0] * b2 + a[i +  3 + aOffset0] * b3;
                    s0b += a[i +  4 + aOffset0] * b4  + a[i +  5 + aOffset0] * b5  + a[i +  6 + aOffset0] * b6 + a[i +  7 + aOffset0] * b7;
                    s1a += a[i      + aOffset1] * b0  + a[i +  1 + aOffset1] * b1  + a[i +  2 + aOffset1] * b2 + a[i +  3 + aOffset1] * b3;
                    s1b += a[i +  4 + aOffset1] * b4  + a[i +  5 + aOffset1] * b5  + a[i +  6 + aOffset1] * b6 + a[i +  7 + aOffset1] * b7;
