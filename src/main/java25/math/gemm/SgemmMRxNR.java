...
public final class SgemmMRxNR {
    ...
    // float
    private static final int MR_Height = 8; // fixed by AVX2 register width
    private static final int NR_Width = 8;  // 16 (constrained by the number of AVX2 registers, 8 should use about 10 registers) 

    private static final int KC = 512;  // 512 .. 1024 [the whole thing must fit into L1]
    private static final int MC = 192;  // 128 .. 256 (256) [with 512 KiB L2]
    ...
    //
    private static void pack_A_8xk(int k, int A_start, float[] A, int incRowA, int incColA, float[] work,
            int work_start) {
    ...
    private static void pack_A_8xk_fast(int k, int A_start, float[] A, int incColA, float[] work,
            int work_start) {
    ...
    //
    private static void pack_B_kx8(int k, int B_start, float[] B, int incRowB, int incColB, float[] work,
            int work_start) {
    ...
    private static void pack_B_kx8_fast(int k, int B_start, float[] B, int incRowB, float[] work,
            int work_start) {
    ...
    static void pack_A(int mc, int kc, int A_start, float[] A, int incRowA, int incColA, float[] work) {
        ...
        for (i = 0; i < mp; ++i) {
            if (incRowA == 1) {
                pack_A_8xk_fast(kc, A_start, A, incColA, work, work_start);
            } else {
                pack_A_8xk(kc, A_start, A, incRowA, incColA, work, work_start);
            }
    ...
    private static void sgemm_micro_kernel(int kc, float alpha, int A_panel_start, float[] A_panel, int B_panel_start,
        ...
        //
        if (beta != 1.0f) {
            sgemm_micro_betaMulC_MR8(beta, C_panel_start, C_panel, incColC);
        }
        ...
        //
        sgemm_micro_plusAlphaAB_MR8(alpha, C_panel_start, C_panel, incColC, AB);
    }

    //
    // Update C <- beta*C
    //
    private static void sgemm_micro_betaMulC_MR8(float beta, int C_panel_start, float[] C_panel,
            int incColC) {
    ...
    //
    private static void sgemm_micro_plusAlphaAB_MR8(float alpha, int C_panel_start, float[] C_panel,
    ...
    public static int sgemm(int rowsA, int colsB, int colsA, float alpha, int offA, float[] A, int incRowA, int incColA,
        ...
        return sgemmRange(rowsA, 0, colsB, colsA, alpha, offA, A, incRowA, incColA,
    ...
    private static void parallelizeOverN(ExecutorService executor, int rowsA, int colsB, int colsA,
            float alpha, int offA, float[] A, int incRowA, int incColA,
            int offB, float[] B, int incRowB, int incColB,
            float beta, int offC, float[] C, int incRowC, int incColC) {

        int taskCount = GemmParallelSupport.taskCountUncapped(colsB, NC);
        if (taskCount <= 1) {
            sgemmRange(rowsA, 0, colsB, colsA, alpha, offA, A, incRowA, incColA,
                    offB, B, incRowB, incColB, beta, offC, C, incRowC, incColC);
            return;
        }
    ...
    //
    private static int sgemmRange(int rowsA, int colsB_from, int colsB_to, int colsA,
            float alpha, int offA, float[] A, int incRowA, int incColA,
            int offB, float[] B, int incRowB, int incColB,
            float beta, int offC, float[] C, int incRowC, int incColC) {
    ...
    private static final class ColumnRangeTask implements Runnable {
        ...
        @Override
        public void run() {
            sgemmRange(rowsA, jFrom, jTo, colsA, alpha, offA, A, incRowA, incColA,
                    offB, B, incRowB, incColB, beta, offC, C, incRowC, incColC);
        }
    }
}
